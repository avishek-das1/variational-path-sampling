!force optimization for Variational Path Sampling, MCVB-T preconditioning
...
!Passive dimer in 2d active bath, Gaussian basis for control force, uses a value baseline
...
subroutine traj(N,r0,theta0,vo,Mr,Mt,Mvr,Mvt,chi,psi,s,omega,delomega,delL,my_id,npercorecount,loopcount)
    ...
    !for enforcing stepmid protocol
    double precision:: steptemp !holder for uniform random float
    integer:: stepmid !converted into the timestep corresponding to tmid
    ...
    !pick tmid from uniform distribution
    call random_number(steptemp)
    stepmid=ceiling(steptemp*steps)
    ...
    do i=1,steps
        ...
        !propagate position without driving force before stepmid
        if (i<stepmid) then
            delr(3:N,:)=dt*F(3:N,:)/gammab+etavarb*eta(3:N,:)
            delr(1:2,:)=dt*F(1:2,:)/gammad+etavard*eta(1:2,:)
        else
