!force optimization for Variational Path Sampling, initialization with well-tempered metadynamics
...
!Passive dimer in 2d active bath, Gaussian basis for control force
!main program, manages the mpi side of things, calls subroutine "traj"
program metadynamics
    use mpi
    implicit none

    double precision,parameter:: s=-100 !bias
    double precision,parameter :: vo = 9.0d0 !magnitude for self-propulsion
    integer, parameter:: N=80,Mr=50,Mt=50 ! Total number of basis is Mr*Mt
    double precision:: chi(Mr,Mt),chirun(Mr,Mt) !coefficients for force as a function of x,t and their running average over optimization steps
    ...
    integer, parameter:: frames=10000 !total number of initial bath configurations
    ...
    do loopcount=1,100
        ...
        if (my_id==0) then
            ...
            if (modulo(loopcount,50)==0) then
                    do i=1,Mr
                        write(120,'(41(2x,E15.4E3))') chirun(i,:)/loopcount
...
subroutine traj(N,r0,theta0,vo,Mr,Mt,Mvr,Mvt,chi,psi,s,omega,delomega,delL,my_id,npercorecount,loopcount)
    ...
    double precision:: chi(Mr,Mt)
    double precision, intent(in):: r0(N,2), theta0(N)
    ...
    !well-temepered metadynamics structures
    double precision:: r_hist(Mr), rmin, rmax, dc
    double precision, parameter:: metaw = 4000,metaT = 9000
    integer:: t_ind,r_ind !return in every timestep
    ...
    do i=1,steps
        ...
        r_ind = ceiling((r12-gaussianr(1,1))*(Mr-1)/(2.3-0.9)) !this should be the instantaneous index of bins defined by the gaussiansx
        if (r_ind>0 .and. r_ind<Mr) then
            r_hist(r_ind) = r_hist(r_ind) + 1.0d0 !histogram it

        if(mod(i,100)==1) then
            t_ind = ceiling(i*1.0d0/(steps/(Mt-1)))
            dc = 10*(dt*(steps-1.0d0)/(Mt-1))*metaw*metaT/(metaT+metaw*r_hist(r_ind))
            do j = t_ind, Mt
                chi(r_ind,j) = chi(r_ind,j) + dc
            enddo
        end if
        endif
    ...
    if (r12<1.85d0) then !didn't reach target region
        delomega=0.0d0
    else
        omega(2)=1.0d0
        delomega=chi
